...
function checkPrettierInstallation() {
  try {
    ...
    const localResult = spawnSync('npx', ['prettier', '--version'], { stdio: 'pipe' });
    ...
    const globalResult = spawnSync('prettier', ['--version'], { stdio: 'pipe' });
    ...
    console.log(`${colors.cyan}npm install --save-dev prettier${colors.reset} or ${colors.cyan}npm install -g prettier${colors.reset}`);
    return false;
  } catch (error) {
...
// Helper function to make output colorful
function colorize(text, color) {
  return `${colors[color]}${text}${colors.reset}`;
}

// Generate ignore patterns argument for prettier
function getIgnorePatterns() {
  // Create an array of ignore patterns using the correct format
  return config.ignoreFolders.map(folder => `--ignore-path "${folder}"`).join(' ');
}
...
function runPrettier(command, dryRun = false) {
  ...
  try {
    ...
    console.error(`${colors.red}Error creating temp ignore file:${colors.reset}`, error.message)
    return
  }
...
const options = [
  ...
  { key: '6', label: 'Configure ignore folders', action: configureIgnoreFolders },
  { key: '7', label: 'Configure default extensions', action: configureDefaultExtensions },
  { key: '8', label: 'Show current configuration', action: showConfiguration },
