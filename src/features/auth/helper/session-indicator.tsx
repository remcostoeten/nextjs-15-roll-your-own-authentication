...
export default function SessionIndicator({ initialState }: AuthIndicatorProps) {
	...
	const featureConfig = useFeatureConfig(user?.role)

	useEffect(() => {
		setMounted(true)
	}, [])

	if (!featureConfig.showSessionIndicator.enabled || !mounted) {
		return null
	}

	const role = user?.role ?? 'guest'
	const roleConfig = ROLE_CONFIGS[role] ?? ROLE_CONFIGS.guest
	const statusDotColor = !isAuthenticated ? 'bg-red-500' : roleConfig.dotColor
		...
		<div className="fixed z-50 bottom-4 right-4">
			<div
				className={`
					...
					hover:border-neutral-700/50 hover:shadow-xl
