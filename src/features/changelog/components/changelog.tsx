...
const isCommit = (
	item: TimelineItem
): item is TimelineItem & { type: 'commit'; data: GitHubCommit } => {
	return item.type === 'commit'
}

const isDeployment = (
	item: TimelineItem
): item is TimelineItem & { type: 'deployment'; data: VercelDeployment } => {
...
const pillStyles = {
	...
	building:
		'bg-yellow-500/20 text-yellow-300 border-2 border-yellow-500/30 shadow-[0_0_15px_rgba(245,158,11,0.15)]',
	error: 'bg-red-500/20 text-red-300 border-2 border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.15)]',
	canceled:
		'bg-neutral-500/20 text-neutral-300 border-2 border-neutral-500/30'
} as const
...
export default function Changelog({ className }: ChangelogProps) {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const [commitsData, deploymentsData, statsData] =
					await Promise.all([
						getGithubCommits(),
						getVercelDeployments(),
						getCommitStats()
	...
	const getTimeRangeData = (
		...
		return Array.from({ length: days }, (_, i) => {
			...
			const commitsOnDay = commits.filter(
				(c) =>
	...
	const filteredCommits = commits
		? commits.filter((commit) => {
				const matchesSearch = commit.commit.message
					.toLowerCase()
				...
				const matchesImpact =
					!showImpactfulOnly || calculateCommitImpact(commit) > 50
				...
				const matchesDate = (() => {
					...
					switch (dateFilter) {
						case 'today':
							return (
								commitDate.toDateString() === now.toDateString()
							)
						case 'week':
			...
			<Tooltip
				content={
					<div className="space-y-2">
						<p>
							Impact Score measures the significance of a commit
							based on:
						</p>
						...
						<p className="text-sm mt-2">
							Higher scores indicate more substantial changes to
							the codebase.
						</p>
					</div>
				}
				...
				<div className="flex items-center gap-2 text-sm">
					<div className="h-2 w-24 bg-black/20 rounded-full overflow-hidden">
						<motion.div
							...
							style={{
								width: `${Math.min(100, (impact / 100) * 100)}%`
							}}
							initial={{ width: 0 }}
							animate={{
								width: `${Math.min(100, (impact / 100) * 100)}%`
		...
		<div className={cn('max-w-page-size mx-auto px-4 py-8', className)}>
			...
			<div className="mb-12">
				...
				<div className="space-y-4 text-neutral-400 max-w-2xl">
					<p>
						Track the evolution of our authentication system through
						a comprehensive timeline of changes, deployments, and
						improvements. This changelog provides detailed insights
						into code changes, feature additions, and system
						deployments.
					</p>
					<p>
						Each commit is analyzed for its impact score - a metric
						that combines the scope of changes (additions and
						deletions) with the breadth of files affected, helping
									...
									<div className="mb-4 p-3 bg-black/20 rounded-lg text-sm text-neutral-400">
										...
										{deployment.meta.commit.sha && (
											<div className="mt-2 text-xs font-mono">
												{deployment.meta.commit.sha.slice(
													0,
													7
												)}
											</div>
				...
				<>
					...
					<div className="flex items-center gap-4 mb-6">
						...
						<div className="flex items-center gap-2">
							<Button
								...
								size="sm"
								onClick={() =>
									setShowImpactfulOnly(!showImpactfulOnly)
								}
								className={cn(
							...
							<Button
								...
								size="sm"
								onClick={() =>
									setShowMergesOnly(!showMergesOnly)
								}
								className={cn(
					...
					<div
						className={cn(
							'space-y-6',
							viewMode === 'grid' &&
								'grid grid-cols-2 gap-6 space-y-0'
						)}
					>
