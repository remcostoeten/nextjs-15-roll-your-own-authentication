		...
		if (sortBy === 'createdAt') {
			query =
				sortDirection === 'asc'
					...
					: query.orderBy(desc(tickets.createdAt));
		} else if (sortBy === 'updatedAt') {
			query =
				sortDirection === 'asc'
					? query.orderBy(asc(tickets.updatedAt))
					: query.orderBy(desc(tickets.updatedAt));
		} else if (sortBy === 'priority') {
			query =
				sortDirection === 'asc'
					? query.orderBy(asc(tickets.priority))
					: query.orderBy(desc(tickets.priority));
		}
		...
		const memberCheck = await db
			...
			.where(
				and(
					...
					eq(workspaceMembers.userId, data.reporterId)
				)
			);
