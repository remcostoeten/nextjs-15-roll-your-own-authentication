					...
					<div className="lg:col-span-2 bg-[#0A0A0A] rounded-xl border border-[#1a1a1a] p-6 backdrop-blur-xl">
						...
						<div className="grid grid-cols-3 gap-4 border-t border-[#1a1a1a] pt-6">
							...
							<div>
								...
								<p className="text-white flex items-center">
									<span
										className={`w-1.5 h-1.5 rounded-full ${user?.role === 'admin' ? 'bg-purple-500' : 'bg-blue-500'} mr-2`}
									></span>
									{user?.role === 'admin'
										? 'Administrator'
										: 'User'}
								...
								<div className="flex items-center space-x-2 mb-4">
									...
									<div>
										...
										<p className="text-sm text-white">
											{metrics.currentLocation?.city},{' '}
											{metrics.currentLocation?.country}
										</p>
									</div>
								</div>
							...
							<div>
								...
								<div className="flex items-center space-x-2">
									...
									<div>
										...
										<p className="text-sm text-white">
											{metrics.device?.screenResolution}
										</p>
									</div>
								</div>
							</div>
						...
						<div className="space-y-4">
							<div>
								...
								<div className="h-1 bg-[#1a1a1a] rounded-full overflow-hidden">
									<div
										...
										style={{
											width: `${Math.min((metrics.navigation.averageTimeOnSite / 3600) * 100, 100)}%`,
										}}
									/>
								</div>
							</div>
							<div>
								...
								<div className="space-y-2">
									{metrics.navigation.mostVisitedPages
										.slice(0, 3)
										.map((page, index) => (
											<div
												key={index}
				...
				<div className="bg-[#0A0A0A] rounded-xl border border-[#1a1a1a] p-6">
					<div className="flex justify-between items-center mb-6">
						...
						<div className="flex space-x-2">
							{['auth', 'navigation', 'error', 'system'].map(
								(type) => (
									<span
										key={type}
										className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-neutral-800 text-neutral-400"
									>
										{type}
									</span>
								)
					...
					<div className="space-y-6">
						{metrics.activityLog.length > 0 ? (
							metrics.activityLog.map((activity) => (
								...
								>
									<div
										className={`absolute left-[-2px] top-[18px] w-[5px] h-[5px] rounded-full ring-4 ring-black
										${
											activity.type === 'auth'
												? 'bg-green-500'
												: activity.type === 'error'
