...
export function DashboardShell({ children }: DashboardShellProps) {
	...
	const NavItem = ({
		...
		icon: Icon,
		children,
	}: { href: string; icon: any; children: React.ReactNode }) => {
		...
		<div className="flex min-h-screen flex-col">
			...
			<header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-background/95 px-4 backdrop-blur supports-[backdrop-filter]:bg-background/60 sm:px-6 lg:hidden">
				...
				<Avatar className="h-8 w-8">
					<AvatarImage
						src=""
						alt={auth.status === 'authenticated' ? auth.user.email : ''}
					/>
					<AvatarFallback>
				...
				>
					...
					<nav className="flex-1 overflow-auto py-4 px-2">
						<div className="space-y-1 px-2">
							...
							<NavItem href="/dashboard/connected-accounts" icon={LinkIcon}>
								Connected Accounts
							</NavItem>
						...
						<div className="flex items-center justify-between mb-8">
							<div>
								<h1 className="text-2xl font-bold tracking-tight">
									...
									{pathname === '/dashboard/connected-accounts' &&
										'Connected Accounts'}
