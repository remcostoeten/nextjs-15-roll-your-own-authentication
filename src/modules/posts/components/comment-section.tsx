...
import { Comment } from "@/server/db/schemas/comments"
...
export function CommentSection({ postId, currentUser }: CommentSectionProps) {
    ...
    const loadComments = useCallback(async () => {
        try {
            ...
            const commentsData = await getComments({ postId, includeReplies: true })
            ...
            {currentUser ? (
                ...
                <div className="bg-background-lighter border border-button-border rounded-lg p-4 text-center mb-8">
                    <p className="text-button">
                        <a href="/login" className="text-[#4e9815] hover:underline">Log in</a> to join the discussion
                    </p>
                </div>
            )}
