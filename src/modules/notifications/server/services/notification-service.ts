...
import { TCreateNotificationInput, TGetNotificationsOptions, TNotificationStats, TNotificationWithActor } from '../../types';
...
export async function createNotification(
  ...
  return await repository.create(data).then(notification => {
    if (data.actorId) {
      return repository.findByUserId(data.userId, { limit: 1 })
