	...
	useEffect(() => {
		if (process.env.NODE_ENV !== 'production') {
			setDebugMetrics([
				{
					label: 'User Role',
					value: auth.status === 'authenticated' ? auth.user?.role || 'Guest' : 'Guest',
				},
				{ label: 'Notification Types', value: Object.keys(settings).length },
				{ label: 'Environment', value: process.env.NODE_ENV || 'development' },
			]);
		}
	}, [settings, auth.status, auth.status === 'authenticated' ? auth.user?.role : undefined]);
		...
		<>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Customize when you receive notifications</CardDescription>
				</CardHeader>
						...
						<div className="space-y-3">
							{[
								...
								},
								{
									id: 'teamMessages',
									label: 'Team Messages',
									description: 'Direct messages and team communications',
								},
								{
									id: 'workspaceInvites',
									label: 'Workspace Invites',
									description: 'Invitations to join workspaces',
								},
								{
									id: 'mentions',
									label: 'Mentions',
								...
								},
								{
									id: 'comments',
									label: 'Comments',
									description: 'Comments on your work or discussions',
								},
								{
									id: 'fileShares',
									label: 'File Shares',
								...
								<div key={id} className="flex items-start justify-between py-2">
									...
									<Switch
										...
										onCheckedChange={() =>
											handleToggle(id as keyof typeof settings)
										}
									/>
								</div>
							))}
