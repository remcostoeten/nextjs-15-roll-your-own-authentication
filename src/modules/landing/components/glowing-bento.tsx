...
const GridItem = ({ area, icon, title, description }: GridItemProps) => {
	return (
		<li className={cn('min-h-56 list-none group/bento', area)}>
			<div className="relative h-full rounded-none border-l border-r border-l border-r border-b-b  p-2 md:p-3">
				<GlowingEffect
					spread={40}
					glow={true}
					disabled={false}
					proximity={64}
					inactiveZone={0.01}
					border-l border-r border-bWidth={3}
				/>
				<div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden  border-l border-r border-b backdrop-blur-sm p-6 shadow-2xl">
					...
					<div className="relative z-10 flex flex-1 flex-col justify-between gap-3">
						<div className="w-fit rounded border-l border-r border-b border-gray-700  backdrop-blur-sm p-2 text-blue-400">
							{icon}
						</div>
