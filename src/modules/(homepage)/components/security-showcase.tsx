  ...
  const TerminalCode = ({ code }: { code: string }) => {
    ...
    return (
      <div className="bg-[#0D0C0C] border border-[#1E1E1E] rounded-md p-6 mt-4 relative overflow-hidden shadow-lg">
        ...
        <pre ref={codeRef} className="text-sm font-mono whitespace-pre-wrap" style={{ minHeight }}>
          {displayedCode.split("\n").map((line, i) => (
      ...
      <div className="container mx-auto px-6 relative z-10 max-w-7xl">
        ...
        >
          ...
          {activeTooltip && tooltipAnchorEl && (
            <div className="tooltip-portal" style={{ position: "relative", zIndex: 100 }}>
              <AnimatePresence>
                <motion.div
                  ref={tooltipRef}
                  ...
                  exit="exit"
                  style={{
                    boxShadow: "0 10px 25px -5px rgba(0, 0, 0, 0.8), 0 0 5px rgba(78, 152, 21, 0.2)",
                    backdropFilter: "blur(8px)",
                    position: "absolute",
                    top: tooltipAnchorEl.getBoundingClientRect().top + window.scrollY + tooltipAnchorEl.offsetHeight,
                    left: tooltipAnchorEl.getBoundingClientRect().left,
                  }}
                >
                  <div className="font-bold text-[#F2F0ED] mb-1 flex items-center">
                    <span className="text-[#4e9815] mr-1">&gt;</span> {tooltips[activeTooltip].title}
                  </div>
                  <div className="text-xs text-[#8C877D] font-mono">{tooltips[activeTooltip].content}</div>
