		...
		auth.status === 'authenticated'
			? [
					...
					'createdAt' in auth.user && {
						icon: Clock,
						label: 'Account Age',
						value: formatDistanceToNow(new Date(auth.user.createdAt as string), {
							addSuffix: true,
						}),
					},
										...
										{auth.status === 'authenticated' && (
											<div className="metrics-container mt-3 space-y-3 rounded-md bg-[rgb(19,19,19)] p-3">
												{metrics.map((metric, i) => (
													<div
														key={i}
														className="metric-item flex items-center space-x-3 text-sm"
													>
														<metric.icon className="metric-icon h-4 w-4 text-gray-400" />
														<span className="metric-label text-gray-400">
															{metric.label}:
														</span>
														<span className="metric-value font-medium text-white">
															{metric.value}
														</span>
													</div>
												))}
