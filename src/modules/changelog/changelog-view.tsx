        ...
        >
          ...
          {groupedEntries.length > 0 ? (
            groupedEntries.map((group) => (
              <ChangelogMonthGroup key={`${group.year}-${group.month}`} group={group} isAdmin={isAdmin} />
            ))
          ) : (
            <div className="text-center py-12">
              <p className="text-[#8C877D]">No changelog entries match your filters.</p>
              <button onClick={() => setActiveFilters([])} className="mt-4 text-sm text-[#4e9815] hover:underline">
                Clear filters
              </button>
            </div>
          )}
