	...
	return (
		<div className="border border-[#1E1E1E] rounded-lg bg-[#0D0C0C]/50 p-4">
			...
			{isLoading ? (
				...
				<ul className="space-y-3">
					{topEntries.map((entry) => (
						<li
							key={entry.id}
							className="border-b border-[#1E1E1E] pb-3 last:border-0"
						>
							<Link
								href={`/changelog#${entry.id}`}
								className="flex items-center justify-between group"
							>
								<span className="text-[#8C877D] group-hover:text-[#F2F0ED] transition-colors">
									{entry.title}
								</span>
			...
			<div className="mt-4 text-center">
				<Link
					href="/changelog"
					className="text-xs text-[#4e9815] hover:underline"
				>
