...
export const featureConfig: FeatureConfig = {
	auth: {
		...
		passwordValidation: {
			enabled: parseEnvBoolean(
				process.env.FEATURE_PASSWORD_VALIDATION,
				true
			),
			minLength: parseEnvNumber(
				process.env.PASSWORD_MIN_LENGTH,
				defaultConfig.auth.passwordValidation.minLength
			),
			requireNumbers: parseEnvBoolean(
				process.env.PASSWORD_REQUIRE_NUMBERS,
				defaultConfig.auth.passwordValidation.requireNumbers
			),
			requireSpecialChars: parseEnvBoolean(
				process.env.PASSWORD_REQUIRE_SPECIAL,
				defaultConfig.auth.passwordValidation.requireSpecialChars
			),
			requireUppercase: parseEnvBoolean(
				process.env.PASSWORD_REQUIRE_UPPERCASE,
				defaultConfig.auth.passwordValidation.requireUppercase
			),
			requireLowercase: parseEnvBoolean(
				process.env.PASSWORD_REQUIRE_LOWERCASE,
				defaultConfig.auth.passwordValidation.requireLowercase
			),
			minimumStrength: parseEnvNumber(
				process.env.PASSWORD_MIN_STRENGTH,
				defaultConfig.auth.passwordValidation.minimumStrength
			)
		}
	},
	profiles: {
	...
	sessionIndicator: {
		enabled: parseEnvBoolean(
			process.env.FEATURE_SESSION_INDICATOR_ENABLED,
			true
		)
	}
}
