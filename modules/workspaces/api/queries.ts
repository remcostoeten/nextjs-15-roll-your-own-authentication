...
import { workspaces, workspaceMembers, users, tasks, workspaceActivities } from "@/server/db/schema"
...
export async function searchWorkspaceMembers(workspaceId: string, query: string) {
  const user = await getCurrentUser()
...
export async function getWorkspaceStats(workspaceId: string) {
  ...
  const [taskCount] = await db.select({ count: count() }).from(tasks).where(eq(tasks.workspaceId, workspaceId))
  ...
  const [inProgressTaskCount] = await db
    .select({ count: count() })
    .from(tasks)
    .where(and(eq(tasks.workspaceId, workspaceId), eq(tasks.status, "in_progress")))

  const [doneTaskCount] = await db
    .select({ count: count() })
    .from(tasks)
    .where(and(eq(tasks.workspaceId, workspaceId), eq(tasks.status, "done")))
