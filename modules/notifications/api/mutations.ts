...
export async function createNotification(formData: FormData) {
  ...
  try {
    ...
    const rawData = {
      ...
      targetUserIds: formData.getAll("targetUserIds").map((id) => Number(id)),
    }
...
export async function markNotificationAsRead(notificationId: number) {
  ...
  try {
    ...
    const existingUserNotification = await db
      ...
      .where(and(eq(userNotifications.userId, user.id), eq(userNotifications.notificationId, notificationId)))
      .limit(1)
...
export async function markAllNotificationsAsRead() {
  ...
  try {
    ...
    await db
      ...
      .where(and(eq(userNotifications.userId, user.id), eq(userNotifications.isRead, false)))
...
export async function deleteNotification(notificationId: number) {
  ...
  try {
    ...
    // Log activity
    await logUserActivity(user.id, "delete_notification", `Deleted notification: ${notification.title}`)
