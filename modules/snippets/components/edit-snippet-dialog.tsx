...
import { updateSnippet } from "@/modules/snippets/api/mutations"
...
export function EditSnippetDialog({ snippet, open, onOpenChange }: EditSnippetDialogProps) {
  ...
  const [selectedCategory, setSelectedCategory] = useState<string | null>(snippet.category?.id || null)
  const [selectedLabels, setSelectedLabels] = useState<string[]>(snippet.labels.map((label) => label.id))
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Extract workspace ID from snippet ID (assuming format like "workspace:id")
        const workspaceId = snippet.id.split(":")[0]
  ...
  useEffect(() => {
    ...
    setSelectedCategory(snippet.category?.id || null)
    setSelectedLabels(snippet.labels.map((label) => label.id))
  ...
  const getLabelById = (id: string) => {
    return labels.find((label) => label.id === id) || snippet.labels.find((label) => label.id === id)
  }
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" role="combobox" className="justify-between">
                    {selectedCategory
                      ? categories.find((category) => category.id === selectedCategory)?.name || snippet.category?.name
                      : "Select category"}
                    ...
                    return (
                      ...
                      >
                        ...
                        <X
                          ...
                          onClick={() => setSelectedLabels(selectedLabels.filter((id) => id !== labelId))}
                        />
                      </Badge>
                    )
                  })}
