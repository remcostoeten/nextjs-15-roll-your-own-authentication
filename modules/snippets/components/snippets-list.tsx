...
import { bulkOperateSnippets, exportSnippets } from "@/modules/snippets/api/mutations"
...
import { FolderArchiveIcon as ArchiveBox, Download, MoreHorizontal, Pin, Star, Tag, Trash2 } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function SnippetsList({
  ...
  const handleSelectSnippet = (id: string, selected: boolean) => {
    if (selected) {
      ...
      setSelectedSnippets((prev) => prev.filter((snippetId) => snippetId !== id))
    }
  }
  ...
  const handleBulkOperation = async (operation: string) => {
    ...
    try {
      ...
      if ((operation === "addLabel" || operation === "removeLabel") && !selectedLabel) {
  ...
  if (snippets.length === 0) {
    return (
      <EmptyState
        ...
        description={
          searchQuery || categoryId || labelIds.length > 0 || isPublicOnly || isPinned || isFavorite || isArchived
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-muted p-2 rounded-md flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => handleOpenLabelDialog("addLabel")}>Add label</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleOpenLabelDialog("removeLabel")}>Remove label</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => handleBulkOperation("pin")}>
                  <Pin className="mr-2 h-4 w-4" />
                  Pin
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleBulkOperation("unpin")}>
                  <Pin className="mr-2 h-4 w-4" />
                  Unpin
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleBulkOperation("favorite")}>
                  <Star className="mr-2 h-4 w-4" />
                  Add to favorites
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleBulkOperation("unfavorite")}>
                  <Star className="mr-2 h-4 w-4" />
                  Remove from favorites
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => handleBulkOperation(isArchived ? "unarchive" : "archive")}>
      ...
      {totalPages > 1 && <Pagination currentPage={page} totalPages={totalPages} baseUrl={`?page=`} />}
      ...
      <Dialog open={showLabelDialog} onOpenChange={setShowLabelDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Choose a label to apply to the selected snippets.</DialogDescription>
          </DialogHeader>
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" role="combobox" className="w-full justify-between">
                  {selectedLabel ? labels.find((label) => label.id === selectedLabel)?.name : "Select label"}
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => handleBulkOperation(selectedLabel ? "addLabel" : "")}
