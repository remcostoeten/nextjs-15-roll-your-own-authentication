...
export function SnippetCard({ snippet, selectable = false, selected = false, onSelectChange }: SnippetCardProps) {
  ...
  const previewContent = snippet.content.length > 200 ? `${snippet.content.substring(0, 200)}...` : snippet.content
    ...
    try {
      ...
      if (result.error) {
        ...
        toast({
          ...
          description: snippet.isPinned ? "Snippet unpinned successfully" : "Snippet pinned successfully",
        })
        router.refresh()
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.error) {
        ...
        toast({
          ...
          description: snippet.isFavorite ? "Snippet removed from favorites" : "Snippet added to favorites",
        })
        router.refresh()
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.error) {
        ...
        toast({
          ...
          description: snippet.isArchived ? "Snippet restored from archive" : "Snippet moved to archive",
        })
        router.refresh()
      }
    } catch (error) {
    ...
    <>
      <Card className={`${snippet.isPinned ? "border-primary/50" : ""}`}>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              {snippet.isPinned && <Pin className="h-4 w-4 text-primary" />}
              {snippet.isFavorite && <Star className="h-4 w-4 text-yellow-500" />}
              {snippet.isPublic && (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem onClick={handleToggleArchive} disabled={isArchiving}>
                    ...
                    {snippet.isArchived ? "Restore from archive" : "Move to archive"}
                  </DropdownMenuItem>
                  {snippet.isPublic && snippet.shareId && (
                    <DropdownMenuItem asChild>
                      <Link href={`/s/${snippet.shareId}`} target="_blank">
        ...
        <CardContent>
          ...
          >
            <div className="bg-muted rounded-md overflow-x-auto max-h-[200px]">
              <SnippetSyntaxHighlighter code={previewContent} language={snippet.language} showLineNumbers={false} />
            </div>
          </Link>
        ...
        <CardFooter className="text-xs text-muted-foreground flex justify-between">
          ...
          {snippet.creator && (
            <span>
              By {snippet.creator.firstName} {snippet.creator.lastName}
            </span>
          )}
        </CardFooter>
      </Card>
