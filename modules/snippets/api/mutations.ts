...
export async function createSnippet(workspaceId: string, formData: FormData) {
	try {
		...
		if (!isMember) {
			return {
				error: "You don't have permission to create snippets in this workspace",
			}
		}
...
export async function bulkOperateSnippets(
	workspaceId: string,
	formData: FormData
) {
	try {
		...
		if (!isMember) {
			return {
				error: "You don't have permission to perform bulk operations in this workspace",
			}
		}
		...
		const snippetIds = rawSnippetIds
			? JSON.parse(rawSnippetIds as string)
			: []
		...
		switch (validatedData.operation) {
			...
			case 'addLabel':
				...
				const existingConnections = await db
					...
					.where(
						inArray(
							snippetLabels.snippetId,
							validatedData.snippetIds
						)
					)
				...
				const existingPairs = new Set(
					existingConnections.map(
						(conn) => `${conn.snippetId}:${conn.labelId}`
					)
				)
				...
				const newEntries = validatedData.snippetIds
					.filter(
						(snippetId) =>
							!existingPairs.has(
								`${snippetId}:${validatedData.labelId}`
							)
					)
					.map((snippetId) => ({
						id: createId(),
...
export async function exportSnippets(workspaceId: string, formData: FormData) {
	try {
		...
		if (!isMember) {
			return {
				error: "You don't have permission to export snippets from this workspace",
			}
		}
...
export async function createCategory(workspaceId: string, formData: FormData) {
	try {
		...
		if (!isMember) {
			return {
				error: "You don't have permission to create categories in this workspace",
			}
		}
...
export async function updateCategory(categoryId: string, formData: FormData) {
	try {
		...
		const isMember = await db.query.workspaceMembers.findFirst({
			where: and(
				eq(workspaceMembers.workspaceId, category.workspaceId),
				eq(workspaceMembers.userId, user.id)
			),
		})
...
export async function createLabel(workspaceId: string, formData: FormData) {
	try {
		...
		if (!isMember) {
			return {
				error: "You don't have permission to create labels in this workspace",
			}
		}
...
export async function updateLabel(labelId: string, formData: FormData) {
	try {
		...
		const isMember = await db.query.workspaceMembers.findFirst({
			where: and(
				eq(workspaceMembers.workspaceId, label.workspaceId),
				eq(workspaceMembers.userId, user.id)
			),
		})
