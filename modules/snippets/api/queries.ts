...
import { eq, and, like, or, inArray, desc, asc, sql, count, isNull, not } from "drizzle-orm"
...
export async function getWorkspaceSnippets({
  ...
  if (searchQuery) {
    conditions.push(or(like(snippets.title, `%${searchQuery}%`), like(snippets.content, `%${searchQuery}%`)))
  }
  ...
  if (sortBy === "position") {
    ...
    if (sortOrder === "desc") {
      query = query.orderBy(desc(snippets.isPinned), desc(snippets.position))
    } else {
      query = query.orderBy(desc(snippets.isPinned), asc(snippets.position))
    }
  ...
  const categoryIds = snippetsResult.map((snippet) => snippet.categoryId).filter(Boolean) as string[]
  ...
  const snippetLabelsMap = snippetLabelsResult.reduce(
    (acc, { snippetId, labelId, labelName, labelColor }) => {
      ...
      acc[snippetId].push({ id: labelId, name: labelName, color: labelColor })
      return acc
    },
...
export async function getSnippetById(snippetId: string) {
  ...
  const snippetLabelsData = snippetLabelsResult.map(({ labelId, labelName, labelColor }) => ({
    id: labelId,
    name: labelName,
    color: labelColor,
  }))
...
export async function getWorkspaceSnippetStats(workspaceId: string) {
  ...
  const [totalSnippets] = await db
    .select({ count: count() })
    .from(snippets)
    .where(and(eq(snippets.workspaceId, workspaceId), eq(snippets.isArchived, false)))
  ...
  const [totalLabels] = await db.select({ count: count() }).from(labels).where(eq(labels.workspaceId, workspaceId))

  const [totalPublic] = await db
    .select({ count: count() })
    .from(snippets)
    .where(and(eq(snippets.workspaceId, workspaceId), eq(snippets.isPublic, true), eq(snippets.isArchived, false)))

  const [totalArchived] = await db
    .select({ count: count() })
    .from(snippets)
    .where(and(eq(snippets.workspaceId, workspaceId), eq(snippets.isArchived, true)))
  ...
  const languageDistribution = await db
    ...
    .from(snippets)
    .where(and(eq(snippets.workspaceId, workspaceId), eq(snippets.isArchived, false), not(isNull(snippets.language))))
...
export async function searchSnippetsAcrossWorkspaces(searchQuery: string, limit = 5) {
  const user = await getCurrentUser()
  ...
  const searchResults = await db
    ...
    .where(
      and(
        ...
        or(like(snippets.title, `%${searchQuery}%`), like(snippets.content, `%${searchQuery}%`)),
      ),
    )
