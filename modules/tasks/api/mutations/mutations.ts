...
import { tasks, workspaceMembers, workspaceActivities } from "@/server/db/schema"
...
export async function updateTask(taskId: string, formData: FormData) {
  ...
  try {
    ...
    // Check if user is a member of the workspace
    const membership = await db.query.workspaceMembers.findFirst({
      where: and(eq(workspaceMembers.workspaceId, task.workspaceId), eq(workspaceMembers.userId, user.id)),
    })
