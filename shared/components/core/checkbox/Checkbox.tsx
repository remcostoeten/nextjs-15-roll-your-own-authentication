...
const Checkbox: React.FC<CheckboxProps> = ({
  ...
  const [uniqueId] = useState<string>(`checkbox-${Math.random().toString(36).substring(2, 11)}`);
  ...
  return (
    <label 
      className={`${styles.checkbox} ${round ? styles.round : ''} ${disabled ? styles.disabled : ''} ${className || ''}`} 
      htmlFor={id}
      style={inlineStyles}
    ...
    >
      ...
      <div className={styles.indicator}>
        ...
        <svg className={styles.checkmark} viewBox="0 0 14 14" aria-hidden="true">
          <path 
      ...
      {label && <span className={styles.label}>{label}{required && <span className={styles.required}>*</span>}</span>}
    </label>
  );
};
