...
export default function ApiAuthenticationClientPage() {
  return (
    ...
    <h1 className="scroll-m-20 text-4xl font-bold tracking-tight">API Authentication</h1>
      ...
      <div className="space-y-4">
        ...
        <p>
          Our authentication system uses JSON Web Tokens (JWT) to authenticate API requests. JWTs are a compact,
          URL-safe means of representing claims to be transferred between two parties. The claims in a JWT are encoded
          as a JSON object that is used as the payload of a JSON Web Signature (JWS) structure, enabling the claims to
          be digitally signed.
        </p>
        <p>
          When a user logs in, a JWT is generated and returned to the client. This token should be included in the
          Authorization header of subsequent API requests to authenticate the user.
        </p>
      </div>

      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Authentication Flow</h2>
        <p>The authentication flow consists of the following steps:</p>
        <ol className="my-6 ml-6 list-decimal [&>li]:mt-2">
          <li>User logs in with their credentials (email/password or OAuth)</li>
          <li>Server validates the credentials and generates a JWT</li>
          <li>JWT is returned to the client and stored in an HTTP-only cookie</li>
          <li>Client includes the JWT in subsequent API requests</li>
          <li>Server validates the JWT and processes the request if valid</li>
        </ol>
      </div>

      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">JWT Structure</h2>
        <p>A JWT consists of three parts separated by dots (.):</p>
        <ul className="my-6 ml-6 list-disc [&>li]:mt-2">
          <li>Header: Contains the token type and the signing algorithm</li>
          <li>Payload: Contains the claims (user data and metadata)</li>
  ...
  "exp": 1516246222
      ...
      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Authenticating API Requests</h2>
        <p>To authenticate API requests, include the JWT in the Authorization header of your HTTP requests:</p>
        <CodeBlock
          language="bash"
          code={`curl -X GET \\
  ...
  -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...'`}
        />
        <p>
          In our Next.js application, the JWT is automatically included in API requests via HTTP-only cookies, so you
          don't need to manually include it in the Authorization header.
        </p>
      </div>

      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Server-Side Authentication</h2>
        <p>
          On the server side, you can use the <code>getCurrentUser</code> function to get the authenticated user:
        </p>
        <CodeTabs
          tabs={[
            {
...
export default async function ProtectedPage() {
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Client-Side Authentication</h2>
        <p>On the client side, you can create a custom hook to get the authenticated user:</p>
        <CodeBlock
          language="tsx"
          code={`"use client"
...
`}
      ...
      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Storage Integration Example</h2>
        <p>Here's an example of how to integrate storage with user authentication:</p>
        <CodeTabs
          tabs={
            [
            {
...
export const storageItemsRelations = relations(storageItems, ({ one }) => ({
  user: one(users, {
    ...
    references: [users.id],
      ...
      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Route Protection</h2>
        <p>
          You can protect routes from unauthorized access using middleware or the <code>requireAuth</code> function:
        </p>
        <CodeTabs
          tabs={[
            {
...
export default async function AdminPage() {
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Database Relations</h2>
...
export const usersRelations = relations(users, ({ many }) => ({
  ...
  storageItems: many(storageItems),
        ...
        />
        <p>When querying related data, you can use Drizzle ORM's query builder:</p>
        <CodeBlock
          language="tsx"
          code={`"use server"
...
export async function getUserFiles() {
  ...
  return files
      ...
      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Security Best Practices</h2>
        <p>Follow these best practices to secure your application:</p>
        <ul className="my-6 ml-6 list-disc [&>li]:mt-2">
          <li>Use HTTP-only cookies to store JWTs to prevent XSS attacks</li>
          <li>Set the Secure flag on cookies in production to ensure they're only sent over HTTPS</li>
          <li>Implement CSRF protection for forms and API endpoints</li>
