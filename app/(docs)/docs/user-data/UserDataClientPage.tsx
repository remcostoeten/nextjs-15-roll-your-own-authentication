...
export default function UserDataClientPage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="scroll-m-20 text-4xl font-bold tracking-tight">User Data</h1>
        <p className="mt-2 text-xl text-muted-foreground">Access user data on the server and client side</p>
      </div>

      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">User Data Structure</h2>
        <p>The user data structure is defined in the database schema:</p>
        <CodeBlock
          language="tsx"
          code={`// Users table
export const users = pgTable("users", {
  ...
  updatedAt: Date;
      ...
      <div className="space-y-4">
        ...
        <p>
          You can access user data on the server using the <code>getCurrentUser</code> or <code>requireAuth</code>{" "}
          functions:
        </p>
        <CodeTabs
          tabs={[
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        ...
        <p>
          To access user data on the client, you can create a custom hook that fetches the user data from an API
          endpoint:
        </p>
        <CodeTabs
          tabs={[
...
export function UserProfile() {
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">Updating User Data</h2>
...
export async function updatePersonalInfo(formData: FormData) {
  try {
    ...
    return { error: "Failed to update personal information" }
      ...
      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">User Sessions</h2>
        <p>User sessions are stored in the database and linked to the JWT token:</p>
        <CodeBlock
          language="tsx"
          code={`// Sessions table
export const sessions = pgTable("sessions", {
  ...
  userAgent: text("user_agent"),
        ...
        />
        <p>
          When a user logs in, a new session is created and the session ID is included in the JWT payload. This allows
          you to invalidate sessions without having to change the JWT secret.
        </p>
      </div>

      <div className="space-y-4">
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">User Activities</h2>
...
export const userActivities = pgTable("user_activities", {
  ...
  createdAt: timestamp("created_at").defaultNow().notNull(),
        ...
        />
        <p>
          You can log user activities using the <code>logUserActivity</code> function:
        </p>
        <CodeBlock
          language="tsx"
